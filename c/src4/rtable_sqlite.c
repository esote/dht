...
static int setup_db(struct rtable_sqlite *rt, const char *filename);
static int db_restrict(sqlite3 *db);
static int db_prepare(struct rtable_sqlite *rt);
static int rtable_sqlite_close(struct rtable *v);
static int rtable_sqlite_store(struct rtable *v, const struct node *n);
static int select_old(struct rtable_sqlite *rt, int dist, bool *insert);
static int delete_node(struct rtable_sqlite *rt, const uint8_t node_id[NODE_ID_SIZE]);
static int insert_node(struct rtable_sqlite *rt, const struct node *n, int dist);
static int rtable_sqlite_closest(struct rtable *v, const uint8_t node_id[NODE_ID_SIZE], struct node closest[K], size_t *len);
...
int
rtable_sqlite(struct rtable *rt, const char *filename, const uint8_t self[NODE_ID_SIZE], bool (*alive)(void *ctx, const struct node *n), void *alive_ctx)
{
	struct rtable_sqlite *priv;

	if ((priv = malloc(sizeof(*priv))) == NULL) {
		return -1;
	}

	if (setup_db(priv, filename) == -1) {
		free(priv);
		return -1;
	}

	memcpy(priv->self, self, sizeof(priv->self));
	priv->alive = alive;
	priv->alive_ctx = alive_ctx;

	rt->close = rtable_sqlite_close;
	rt->store = rtable_sqlite_store;
	rt->closest = rtable_sqlite_closest;
	rt->priv = priv;
...
static int
setup_db(struct rtable_sqlite *rt, const char *filename)
{
...
static int
db_prepare(struct rtable_sqlite *rt)
{
...
static int
rtable_sqlite_close(struct rtable *v)
{
	struct rtable_sqlite *rt = v->priv;
	int ret = 0;
...
static int
rtable_sqlite_store(struct rtable *v, const struct node *n)
{
	struct rtable_sqlite *rt = v->priv;
	bool insert = false;
...
static int
select_old(struct rtable_sqlite *rt, int dist, bool *insert)
{
	sqlite3_stmt *stmt;
	struct node row;
	bool was_full = false;
...
static int
delete_node(struct rtable_sqlite *rt, const uint8_t node_id[NODE_ID_SIZE])
{
	sqlite3_stmt *stmt;
...
static int
insert_node(struct rtable_sqlite *rt, const struct node *n, int dist)
{
	sqlite3_stmt *stmt;
...
static int
rtable_sqlite_closest(struct rtable *v, const uint8_t node_id[NODE_ID_SIZE], struct node closest[K], size_t *len)
{
	struct rtable_sqlite *rt = v->priv;
	sqlite3_stmt *stmt;
	int dist;
